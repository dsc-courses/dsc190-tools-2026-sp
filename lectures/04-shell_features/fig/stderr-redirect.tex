...
% screen on the upper right, file on the lower right
\node[lbl, right=2.5cm of program, yshift=0.6cm] (screen) {screen};
\node[lbl, right=2.5cm of program, yshift=-0.6cm] (file) {errors.txt};

% arrows
\draw[->] (stdin) -- node[above, font=\sffamily\small] {STDIN} (program);
\draw[->] (args) -- node[right, font=\sffamily\small] {args} (program);
\draw[->] (program.east) ++(0,0.2) -- node[above, font=\sffamily\small, pos=0.6] {STDOUT} (screen.west);
